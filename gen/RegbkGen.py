...
class RegbkGen(SrcGen):
    ...
    def RdataStr(self, reg, _slice, w, ind):
        ...
        return s 
    def WdataSeqStr(self, reg, _slice, rw=None, ind=None):
        #TODO slice dependent, now it only pad the MSB and it's usually the case
        ...
        s += f'{ind.b}end\n'
        return s
    def WdataCombStr(self, reg, _slice, rw=None, ind=None):
        #TODO slice dependent, now it only pad the MSB and it's usually the case
        ind = self.cur_ind.Copy() if not ind else ind
    ...
    def LogicToClip(self, pkg=None, toclip=True, ind=None):
        ...
        for reg in self.regbk.addrs.enumls:
            bw,tp = gettpbw(reg)
            s += self.LogicStr( w, reg.name.lower(), bw, tp, ind)
    ...
    def WdataToClip(self, pkg=None, toclip=True, ind=None):
        ...
        for reg in self.regbk.addrs.enumls:
            ...
            s += self.WdataCombStr( reg.name, _slice, rw, ind)
            s += self.WdataSeqStr( reg.name, _slice, rw, ind) + '\n'
        if toclip:
            ToClip(s)
        self.regbk = regbktemp
        return s
