...
AlexBest = ModelConf(
    [
        LayerConf(
            227, 227, 11, 11, 55, 55, 4, 3, 64, 4, 2, 1, 4, 4, 64, 14, 14, 4, 1, 2, 3, 4
        ),
        LayerConf(
            31, 31, 5, 5, 27, 27, 1, 64, 256, 4, 1, 1, 4, 4, 64, 27, 27, 4, 4, 4, 2, 2
        ),
        LayerConf(
            15, 15, 3, 3, 13, 13, 1, 256, 256, 4, 1, 1, 4, 4, 64, 13, 13, 4, 4, 4, 3, 1
        ),
        LayerConf(
            15, 15, 3, 3, 13, 13, 1, 256, 256, 4, 1, 1, 4, 4, 64, 13, 13, 4, 4, 4, 3, 1
        ),
...
def WeightData(pch, r, m, s, wb, ab, th):
    ...
    datas, Quants, uQuants, Repacks, Packs = zip(
        *[onefilter(pch, r, m, s, wb, ab, th) for i in range(16)]
    )
...
def BufData(conf, allgb=0):
    ...
    def ibuf_r(bus):
        for i in range(ibuf_read if allgb == 0 else 1):
            bus.values = np.random.randint(
                np.iinfo(np.uint16).max, size=1, dtype=np.uint16
            )
            yield bus.values
    ...
    def gbuf_r(bus):
        for i in range(gb_read // 4 if allgb == 0 else gb_all_read // 4):
            bus.values = np.random.randint(
                np.iinfo(np.uint32).max, size=2, dtype=np.uint32
            )
            yield bus.values
