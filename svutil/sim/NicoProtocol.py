...
class ProtoCreateBus:
    ...
    def ArgParse(self, protoCallback, portCallback, *args, clk, **kwargs):
        ...
        if len(args) == 1:
            protolist = (
                portCallback(kw["name"], hier=kw["hier"])
                if kw.get("hier")
                else portCallback(kw["name"])
            )
            kw.pop("name")
            if kw.get("hier"):
                kw.pop("hier")
...
class NicoAhbBus(ProtoCreateBus):
    ...
    def PortParse(self, name, hier=""):
        ...
        self.haddr = CreateBus(
            (
                hier,
                name + "_haddr",
            )
        )
        self.hwrite = CreateBus(
            (
                hier,
                name + "_hwrite",
            )
        )
        self.htrans = CreateBus(
            (
                hier,
                name + "_htrans",
            )
        )
        self.hsize = CreateBus(
            (
                hier,
                name + "_hsize",
            )
        )
        self.hburst = CreateBus(
            (
                hier,
                name + "_hburst",
            )
        )
        self.hready = CreateBus(
            (
                hier,
                name + "_hready",
            )
        )
        self.hresp = CreateBus(
            (
                hier,
                name + "_hresp",
            )
        )
        self.hrdata = CreateBus(
            (
                hier,
                name + "_hrdata",
            )
        )
        self.hwdata = CreateBus(
            (
                hier,
                name + "_hwdata",
            )
...
class MySlaveTwoWire(TwoWire.Slave):
    def __init__(
        self, rdy: Bus, ack: Bus, data: Bus, clk: int, A=1, B=5, callbacks=list()
    ):
